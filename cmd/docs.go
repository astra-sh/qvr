...
This is the v4 home of what was previously ` + "`qvr sync`" + ` — the sync verb
is now reserved for project reconciliation against the lock file.`,
	RunE: runDocs,
}

func init() {
	docsCmd.Flags().StringVarP(&docsOutputPath, "output-file", "o", "AGENTS.md",
		"path to write the generated file (relative to cwd)")
	docsCmd.Flags().BoolVar(&docsGlobal, "global", false,
...
func runDocs(cmd *cobra.Command, args []string) error {
	...
	lock, err := model.ReadLockFile(model.DefaultLockPath(projectRoot, config.Dir(), docsGlobal))
	if err != nil {
		return fmt.Errorf("read lock: %w", err)
	}
	outPath := docsOutputPath
...
// outPath. Used directly by `qvr docs` and by switch/upgrade/pull (via
// refreshAgentsMDIfPresent) to keep the file fresh when it already exists.
func writeAgentsMD(outPath string, entries []*model.LockEntry) error {
	sort.Slice(entries, func(i, j int) bool { return entries[i].Name < entries[j].Name })

	var b strings.Builder
	b.WriteString("# Agents\n\n")
	b.WriteString("Skills available to agents in this project. Managed by [Quiver](https://github.com/astra-sh/qvr).\n\n")
	if len(entries) == 0 {
...
// the file already exists. Callers use this to keep the description cache in
// sync without silently creating an AGENTS.md for users who haven't opted in.
