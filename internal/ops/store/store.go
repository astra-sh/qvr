...
type Store interface {
	...
	// PutAnnotation appends a human verdict on a session (the outer-loop
	// feedback signal); ListAnnotations reads them back, newest-first. Unlike
	// spans/session_meta, annotations are NOT regenerated by rederive — they are
	// durable human input. See annotations.go + migration 0008.
	PutAnnotation(ctx context.Context, a *AnnotationRow) error
	ListAnnotations(ctx context.Context, f *AnnotationFilter) ([]*AnnotationRow, error)

	// PutEvalRun records one eval run (verdict + per-case rows), keyed by
	// {skill_name, skill_commit} — the lock × evidence join. ListEvalRuns reads
	// them back newest-first for the gate and for lineage. See eval_runs.go +
	// migration 0009.
	PutEvalRun(ctx context.Context, r *EvalRunRow) (int64, error)
	ListEvalRuns(ctx context.Context, f *EvalRunFilter) ([]*EvalRunRow, error)
