...
// seedBareFromFiles writes an arbitrary file map (repo-relative path → content)
// into a fresh repo on the "main" branch and returns a bare clone path usable
// as a worktree source. Unlike setupBareRegistry it makes no layout assumptions,
// so it can model multi-skill repos (root SKILL.md + siblings + app code).
func seedBareFromFiles(t *testing.T, files map[string]string) string {
	t.Helper()
	remoteBare := filepath.Join(t.TempDir(), "remote.git")
	if _, err := gogit.PlainInit(remoteBare, true); err != nil {
		t.Fatalf("init remote bare: %v", err)
	...
	return bareCloneFor(t, remoteBare)
}
